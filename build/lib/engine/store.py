...
STATE_ENV = "HARNESS_STATE_DIR"

# Where the store used to default to. Kept only so a store already sitting there can be
# NOTICED rather than silently orphaned — see `_refuse_a_stranded_store`.
LEGACY_STATE_DIR = REPO_ROOT / "state"


def state_dir() -> Path:
    """Where the store lives, resolved at call time.

    WHY NOT NEXT TO THE ENGINE. The default used to be `<engine>/../state`, which is fine for
    someone developing the engine and wrong for everyone else. Installed into site-packages
    that directory is frequently unwritable; where it IS writable, it puts a user's run ledger
    inside a package directory that the next upgrade replaces. A base that stores a consumer's
    data inside its own installation is not installable.

    `XDG_STATE_HOME` is the specification's answer for exactly this kind of data: it persists
    between runs, it is not configuration, and it is not a cache that may be deleted. macOS has
    no native XDG location, and taking a dependency to compute a per-platform one is the wrong
    trade for a base whose only external requirement is a YAML parser — so one rule is used
    everywhere, and the environment variable is there for anyone who disagrees.
    """
...
class StoreStranded(StoreUnusable):
    def __init__(self, legacy: Path, now: Path):
        super().__init__(
            f"a store exists at the OLD default location and would now be ignored:\n"
            f"  old: {legacy}\n"
            f"  new: {now}\n"
            f"  The default moved out of the engine's own directory, because a package\n"
            f"  directory is often unwritable and is replaced on upgrade.\n"
            f"  Pick one, explicitly:\n"
            f"    keep using it   ->  export {STATE_ENV}={legacy}\n"
            f"    move it across  ->  mkdir -p {now} && mv {legacy}/harness.db* {now}/\n"
            f"    start fresh here ->  {STATE_ENV}={now} harness init   (after which the new\n"
            f"                         store exists and this stops firing)\n"
            f"  Refusing rather than choosing: silently reading the old one would make this\n"
            f"  change a no-op, and silently reading the new one would answer every query as\n"
            f"  if the recorded runs had never happened."
        )
        self.legacy, self.now = legacy, now


def _refuse_a_stranded_store() -> None:
    """Notice a store left behind by the default move, instead of orphaning it.

    Only ever fires when NOBODY said where the store is (no override) and there is something
    to lose: a database at the old location and none at the new one. Both of those together
    describe exactly one situation — a source checkout that predates the move — and that is
    also the only audience able to act on it. A packaged install has no such directory, so a
    consumer never sees this.
    """
    if os.environ.get(STATE_ENV, "").strip():
        return  # they said where; there is nothing to guess about
    legacy = LEGACY_STATE_DIR / "harness.db"
    if not legacy.is_file():
        return
    if db_path().is_file():
        return  # the new one exists too — the move already happened
    raise StoreStranded(LEGACY_STATE_DIR, state_dir())
